...
impl PdClient for RpcClient {
    ...
    fn upload_ru_metrics(&self, req: TokenBucketsRequest) -> PdFuture<()> {
        info!("upload ru metrics to PD"; "req" => ?req);
        let executor = |client: &Client, req: TokenBucketsRequest| {
            let mut inner = client.inner.wl();
            if let Either::Left(ref mut left) = inner.rg_sender {
                let sender = left.take().expect("expect upload_ru_metrics sink");
                let (tx, rx) = mpsc::unbounded();
                ...
                inner.client_stub.spawn(async {
                    ...
                    debug!("upload_ru_metrics stream exited");
                });
                inner.client_stub.spawn(async move {
                    let mut sender = sender.sink_map_err(Error::Grpc);
                    let result = sender
                    ...
                    match result {
                        Ok(()) => {
                            sender.get_mut().cancel();
                            info!("cancel upload_ru_metrics sender");
                        }
                        Err(e) => {
                            error!(?e; "failed to upload_ru_metrics buckets");
                        }
                    };
                });
            }
            ...
            let sender = inner
                ...
                .as_mut()
                .right()
                .expect("expect upload_ru_metrics sender");
            let ret = sender
                .unbounded_send(req)
                .map_err(|e| Error::StreamDisconnect(e.into_send_error()));
            Box::pin(future::ready(ret)) as PdFuture<_>
